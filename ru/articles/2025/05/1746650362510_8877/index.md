...
- `ENC_SSL_CIPHER` - алгоритм симметричного шифрования SSL. По умолчанию: `aes-256-cbc`.
...
```bash
f='DB_NAME.sql.xz.ssl'; p='SECRET'; openssl enc -aes-256-cbc -in "${f}" -out "${f%.*}" -pass "pass:${p}" -d -md 'sha512' -iter '65536' -salt -pbkdf2
```
